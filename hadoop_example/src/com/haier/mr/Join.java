...
import org.apache.hadoop.util.ToolRunner;
 ...
 public class Join extends Configured
   implements Tool
 {
     ...
     throws Exception
   {
     ...
     for (int i = 0; i < args.length; i++) {
       try {
         if ("-m".equals(args[i])) {
           i++; num_maps = Integer.parseInt(args[i]);
         } else if ("-r".equals(args[i])) {
           i++; num_reduces = Integer.parseInt(args[i]);
         } else if ("-inFormat".equals(args[i])) {
           i++; inputFormatClass = Class.forName(args[i]).asSubclass(InputFormat.class);
         }
         else if ("-outFormat".equals(args[i])) {
           i++; outputFormatClass = Class.forName(args[i]).asSubclass(OutputFormat.class);
         }
         else if ("-outKey".equals(args[i])) {
           i++; outputKeyClass = Class.forName(args[i]).asSubclass(WritableComparable.class);
         }
         else if ("-outValue".equals(args[i])) {
           i++; outputValueClass = Class.forName(args[i]).asSubclass(Writable.class);
         }
     ...
     FileOutputFormat.setOutputPath(jobConf, new Path((String)otherArgs.remove(otherArgs.size() - 1)));
     ...
     jobConf.set("mapred.join.expr", CompositeInputFormat.compose(op, inputFormatClass, (Path[])plist.toArray(new Path[0])));
 ...
 * Qualified Name:     org.apache.hadoop.examples.Join
